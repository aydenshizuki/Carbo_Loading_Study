...
# 2. What are the top 5 brands in each commodity?
#Rate top brand by: volume of sales, money earned, number of distinct households that bought
#the product
...
write.csv(sales, "./data/brand_sales.csv")
write.csv(units, "./data/brand_units.csv")

household_aware <-
  transactions %>%
  select(commodity, brand, household) %>%
  unique() %>%
...
write.csv(household_aware, "./data/brand_household.csv")
...
write.csv(returns, "./data/brand_returns.csv")
